...
def main():  
    ...
    caminho_arquivo = caixadetexto.get()
    # Verifica se o caminho fornecido é um diretório válido
    if os.path.isdir(caminho_arquivo):
        # Verifica se o caminho termina com uma barra invertida e adiciona uma se necessário
        if not caminho_arquivo.endswith('\\'):
            caminho_arquivo += '\\'
    
        # Define o nome do arquivo de saída
        input_wave_name = caminho_arquivo + 'audio_gravado.wav'
    else:
        # Exibe uma mensagem de erro se o caminho não for válido
        status_label.config(text="Erro: Por favor, insira um caminho de diretório válido. Exemplo: 'G:\\'")
        input_wave_name = caminho_arquivo
...
# Label para a caixa de texto
caixadetexto_label = Label(window, text="Coloque um local para salvar o arquivo:")  # Título do slider
caixadetexto_label.place(x=20, y=130)  # Posição do label do slider

caixadetexto = Entry(window,width = 33)
caixadetexto.place(x=20, y=150) 
caixadetexto.insert(0, 'Exemplo: G:\\')  # Reinsere o texto padrão se o campo estiver vazio
...
question_menu.place(x=20, y=200)  # Posição do menu de opções

# Botão para iniciar o processo principal
btn_start = Button(window, text="Começar gravação", command=main)
btn_start.place(x=20, y=245)  # Posição do botão
